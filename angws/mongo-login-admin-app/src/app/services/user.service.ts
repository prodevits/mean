...
updateMyProfile(name:string,city:String,mobile_no:string):Observable<{status:string,message:string}>{
 ...
 changePassword(cpass:string,npass:String):Observable<{status:string,message:string}>{
    const data={password:cpass,newpassword:npass};
    return this.httpClient.put<{status:string,message:string}>(this.urlUsers+"/changepass/"+this.userid,data);
 }
